...
const ICONS = {
  ...
  records: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6"><rect x="4" y="3" width="16" height="18" rx="2"/><path d="M8 8h8M8 12h8M8 16h5"/></svg>,
...
export const Sidebar = () => {
  ...
  return (
    <nav className="w-[76px] shrink-0 bg-[var(--bg-page)] border-r border-[var(--border-color)] flex flex-col items-center py-5 gap-2">
      <div className="w-11 h-11 rounded-full border-2 border-[var(--accent)] flex items-center justify-center text-xl mb-4">◆</div>
      {links.map(l => (
        <NavLink
          key={l.path}
          to={l.path}
