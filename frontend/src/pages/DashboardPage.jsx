...
export const DashboardPage = () => {
  ...
  return (
    <div className="h-screen text-sm p-4 overflow-hidden flex flex-col">
      <Navbar title="Store Security Dashboard" subtitle="Live CCTV Recognition & Anomaly Alerts" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 flex-1 min-h-0">
        ...
        <div className="md:col-span-2 relative bg-[var(--bg-panel)] rounded-xl border border-[var(--border-color)] overflow-hidden flex items-center justify-center">
          ...
          {toastAlert && (
            <div
              className={`absolute top-3 left-3 right-3 border-l-4 rounded px-3 py-2 shadow-lg backdrop-blur-sm z-10 transition ${
              ...
              {alerts.length > 0 ? (
                alerts.map((a, i) => {
                  ...
                  const snapshotUrl = a.snapshot_filename
                    ? `/snapshots/${a.snapshot_filename}?token=${encodeURIComponent(token)}`
                    : null;
                  ...
                  return (
                    <div
                      ...
                      className={`border-l-4 ${
                        PRIORITY_COLOR[a.priority] || 'border-slate-500'
