...
export default function Settings() {
    ...
    async function loadSettings() {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          throw new Error(
            err.detail || `Failed to load settings: ${res.status}`
          );
        }
        ...
        setMinMatchingPhotos(
          Number(data.min_matching_photos) || 2
        );

        setMatchDistanceThreshold(
          data.match_distance_threshold ?? ''
        );

        setOverstayLow(
          Number(data.overstay_low_threshold_min) || 30
        );

        setOverstayMedium(
          Number(data.overstay_medium_threshold_min) || 60
        );

        setAlertDedupeWindow(
          Number(data.alert_dedupe_window_sec) || 60
        );

        setEscalationLowToMedium(
          Number(data.escalation_low_to_medium_sec) || 1800
        );

        setEscalationMediumToHigh(
          Number(data.escalation_medium_to_high_sec) || 3600
        );
      } catch (err) {
    ...
    async function loadStoreHours() {
      ...
      try {
        ...
        if (!res.ok) {
          const err = await res.json().catch(() => ({}));
          throw new Error(
            err.detail || `Failed to load store hours: ${res.status}`
          );
        }
  ...
  function resetNotifications() {
    ...
    Promise.all([
      saveSetting(
        'notify_motion',
        DEFAULT_NOTIFICATIONS.notifyMotion
      ),
      saveSetting(
        'notify_person',
        DEFAULT_NOTIFICATIONS.notifyPerson
      ),
    ]).catch((err) => {
      console.error(err);

      showMessage(
        err.message || 'Failed to reset notifications.'
      );
    });
  }
  ...
  function resetThresholds() {
    setMinMatchingPhotos(
      DEFAULT_THRESHOLDS.minMatchingPhotos
    );

    setMatchDistanceThreshold(
      DEFAULT_THRESHOLDS.matchDistanceThreshold
    );

    setOverstayLow(
      DEFAULT_THRESHOLDS.overstayLow
    );

    setOverstayMedium(
      DEFAULT_THRESHOLDS.overstayMedium
    );

    setAlertDedupeWindow(
      DEFAULT_THRESHOLDS.alertDedupeWindow
    );

    setEscalationLowToMedium(
      DEFAULT_THRESHOLDS.escalationLowToMedium
    );

    setEscalationMediumToHigh(
      DEFAULT_THRESHOLDS.escalationMediumToHigh
    );
    ...
    Promise.all([
      ...
      showMessage(
        err.message || 'Failed to reset thresholds.'
      );
    });
  }
  ...
  function resetStoreHours() {
    setStoreOpenTime(
      DEFAULT_STORE_HOURS.storeOpenTime
    );

    setStoreCloseTime(
      DEFAULT_STORE_HOURS.storeCloseTime
    );
    ...
    fetch(`${API_URL}/settings`, {
      ...
      .then(async (res) => {
        if (!res.ok) {
          ...
          throw new Error(
            err.detail ||
              `Failed to reset store hours: ${res.status}`
          );
        }
      })
      .catch((err) => {
        ...
        showMessage(
          err.message || 'Failed to reset store hours.'
        );
      });
  }
  ...
  function saveThresholds(event) {
    ...
    Promise.all([
      ...
      showMessage(
        err.message || 'Failed to save alert thresholds.'
      );
    });
  }
    ...
    fetch(`${API_URL}/settings`, {
      ...
      .then(async (res) => {
        if (!res.ok) {
          ...
          throw new Error(
            err.detail ||
              `Failed to save store hours: ${res.status}`
          );
        }
      })
      .catch((err) => {
        ...
        showMessage(
          err.message || 'Failed to save store hours.'
        );
      });
  }
  ...
  async function clearRecords() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          err.detail || 'Failed to clear records'
        );
      }
      ...
      showMessage(
        `Cleared ${data.alerts_deleted} alert(s) and ${data.incidents_deleted} incident(s).`
      );
    } catch (err) {
      console.error(err);

      showMessage(
        err.message || 'Failed to clear records.'
      );
    } finally {
  ...
  const searchIndex = [
    {
      ...
      title: 'Camera alerts',
      description:
        'Notify when a camera is tampered with or goes offline.',
    },
    {
      category: 'notifications',
      title: 'Person alerts',
      description:
        'Notify on unrecognized people, early arrivals, or overstays.',
    },
    ...
    ...(canSeeThresholds
      ? [
          {
            category: 'thresholds',
            title: 'Minimum matching photos',
            description:
              'How many photo matches are required to confirm an identity.',
          },
          {
            category: 'thresholds',
            title: 'Match distance threshold',
            description:
              'How close a face match must be to count as a match.',
          },
          {
            category: 'thresholds',
            title: 'Overstay thresholds',
            description:
              'Minutes past shift end before an overstay is flagged low/medium/high.',
          },
          {
            category: 'thresholds',
            title: 'Alert dedupe window',
            description:
              'How long repeated alerts for the same person are grouped into one incident.',
          },
          {
            category: 'thresholds',
            title: 'Escalation thresholds',
            description:
              'How long an unacknowledged incident sits before its priority escalates.',
          },
        ]
      : []),
      ...
      ? [
          {
            ...
            title: 'Store hours',
            description:
              'Configure opening and closing time used for after-hours detection.',
          },
        ]
      : []),
    ...
    },
    {
      category: 'account',
      title: 'Name',
      description: 'View your account name.',
    },
    {
      category: 'account',
      title: 'Role',
      description: 'View your assigned system role.',
    },
      ...
      ? [
          {
            ...
            description:
              'Remove all logged alerts and incidents.',
          },
        ]
      : []),
  ];
        ...
        {search.trim() ? (
          <div className="vscode-search-results">
            ...
            {searchResults.map((result, index) => (
              <button
                ...
                onClick={() =>
                  openSearchResult(result.category)
                }
              >
                <div>
          ...
          <div className="vscode-settings-body">
            ...
            <main className="vscode-settings-content">
                ...
                <form onSubmit={saveNotifications}>
                  ...
                  <div className="vscode-setting-row">
                    <div>
                      ...
                      <p>
                        Notify when a camera is tampered with,
                        obstructed, or goes offline.
                      </p>
                    </div>
                  ...
                  <div className="vscode-setting-row">
                    <div>
                      ...
                      <p>
                        Notify on unrecognized people,
                        early arrivals, and overstays.
                      </p>
                    </div>
                  ...
                  <div className="vscode-form-actions">
                    ...
                    {!confirmResetNotifications ? (
                      <button
                        ...
                        onClick={() =>
                          setConfirmResetNotifications(true)
                        }
                      >
                        Reset to default
                      </button>
                      ...
                      <div className="vscode-confirm-group">
                        ...
                        <span className="vscode-confirm-text">
              ...
              {activeCategory === 'thresholds' &&
                canSeeThresholds && (
                  <form onSubmit={saveThresholds}>
                    ...
                    <p className="vscode-description">
                      These control the actual face-recognition
                      matching, overstay severity, incident
                      deduplication, and escalation timing
                      used by the backend.
                    </p>
                    ...
                    <div className="vscode-two-fields">
                      ...
                      <div className="vscode-field">
                        <label>
                          Overstay — low threshold (minutes)
                        </label>
                      ...
                      <div className="vscode-field">
                        <label>
                          Overstay — medium threshold (minutes)
                        </label>
                    ...
                    <div className="vscode-two-fields">
                      ...
                      <div className="vscode-field">
                        <label>
                          Escalate low → medium after (seconds)
                        </label>
                      ...
                      <div className="vscode-field">
                        <label>
                          Escalate medium → high after (seconds)
                        </label>
                    ...
                    <div className="vscode-form-actions">
                      ...
                      {!confirmResetThresholds ? (
                        <button
                          ...
                          className="vscode-outline-btn"
                          onClick={() =>
                            setConfirmResetThresholds(true)
                          }
                        >
                          Reset to default
                        </button>
              ...
              {activeCategory === 'store-hours' &&
                isAdmin && (
                  <form onSubmit={saveStoreHours}>

                    <p className="vscode-description">
                      Used to determine after-hours detection
                      — an unrecognized person seen outside
                      these hours triggers a high-priority
                      stranger alert.
                    </p>
                    ...
                    <div className="vscode-form-actions">
                      ...
                      {!confirmResetStoreHours ? (
                        <button
                          ...
                          className="vscode-outline-btn"
                          onClick={() =>
                            setConfirmResetStoreHours(true)
                          }
                        >
                          Reset to default
                        </button>
                  ...
                  <section className="vscode-danger-section">
                    ...
                    <div className="vscode-danger-row">
                      ...
                      <div>
                        ...
                        <p>
                          Permanently deletes every logged
                          alert and incident from the database.
                        </p>
                      </div>
